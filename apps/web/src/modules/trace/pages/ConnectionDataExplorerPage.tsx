...
import { useConnectionTabPanel } from '../hooks/useConnectionTabPanel';

const TABS = [
  { id: 'inbound',    label: 'Inbound',           icon: Inbox,    description: 'Raw payloads received from source app' },
  { id: 'replica',    label: 'Replica',            icon: Database, description: 'Vendor objects stored in replica layer' },
  { id: 'normalized', label: 'Normalization',      icon: Layers,   description: 'Canonical entities in normalized form' },
  { id: 'outbound',   label: 'Outbound',           icon: Database, description: 'Payloads sent to the destination app' },
] as const;
...
function TabPanel({
...
}: { readonly tabId: TabId; readonly connectionId: string; readonly workspaceId: string }) {
  ...
  const {
    ...
    handleDelete,
    load
  } = useConnectionTabPanel({ tabId, connectionId, workspaceId });
...
export function ConnectionDataExplorerPage() {
  const { id: workspaceId, connectionId } = useParams<{ id: string; connectionId: string }>();
  const [activeTab, setActiveTab] = useState<TabId>('inbound');

  if (!workspaceId || !connectionId) return <Navigate to="/dashboard" replace />;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold tracking-tight">Connection Data</h2>
          <p className="text-sm text-muted-foreground">Inspect raw payloads and data specific to this connection.</p>
        </div>
      </div>
      ...
      <TabPanel key={`${activeTab}-${connectionId}`} tabId={activeTab} connectionId={connectionId} workspaceId={workspaceId} />
    </div>
  );
}