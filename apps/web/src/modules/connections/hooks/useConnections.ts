...
export function useConnections() {
    ...
    const connect = useCallback(
        ({ providerName, clientId, clientSecret, env }: { providerName: string; clientId: string; clientSecret: string; env?: string }) => {
            ...
            let popupUrl = `${apiUrl}/connectors/${providerName}?clientId=${encodeURIComponent(clientId)}&clientSecret=${encodeURIComponent(clientSecret)}&tenantId=${encodeURIComponent(user?.organizationId ?? '')}`;
            if (env) {
