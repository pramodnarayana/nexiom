...
if (!SYSTEM_TENANT_ID || !OWNER_ROLE_ID || !ADMIN_ROLE_ID || !MEMBER_ROLE_ID) {
  // We log a warning instead of crashing immediately, allowing for partial envs in some contexts if needed
  // But strictly, these are required for Identity to function.
  // console.warn('Missing Identity Environment Variables in apps/api/constants.ts');
  // Actually, for type safety, lets throw if we expect string!
}
