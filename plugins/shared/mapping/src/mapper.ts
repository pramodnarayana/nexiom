/**
 * Mapper — Standard Execution Engine.
 *
...
function setNestedValue(obj: Record<string, unknown>, path: string, value: unknown): void {
  ...
  for (const part of parts) {
    if (!isSafeSegment(part)) {
      throw new Error(
        `Mapper: unsafe path segment "${part}" in destPath "${path}". ` +
...
function advanceOrCreate(
  ...
  if (current[part] === undefined || current[part] === null) {
    ...
    throw new TypeError(
      `Mapper: intermediate key "${part}" in "${fullPath}" already holds a ` +
...
export class Mapper {
   ...
   * Worker processes reuse one Mapper instance across a batch,
    ...
    for (const rule of mappingRules) {
      ...
      if (rule.expression !== undefined) {
        if (typeof rule.expression !== 'string' || rule.expression.trim() === '') {
          warnings.push(
            `Mapper: invalid or empty expression for destPath "${rule.destPath}" ` +
              `— field skipped.`,
          );
          continue;
        }
        ...
        try {
          ...
          warnings.push(
            `Mapper: invalid JSONata expression for destPath "${rule.destPath}" ` +
        ...
        try {
          ...
          warnings.push(
            `Mapper: JSONata evaluation failed for destPath "${rule.destPath}" ` +
              `— ${(err as Error).message}. Field skipped.`,
          );
          continue;
        }
        ...
        if (value === undefined) {
          warnings.push(
            `Mapper: expression for destPath "${rule.destPath}" evaluated to ` +
        ...
        if (value === undefined) {
          warnings.push(
            `Mapper: srcPath "${rule.srcPath}" resolved to undefined — field skipped.`,
          );
          continue;
        }
      }
      ...
      try {
        ...
        warnings.push(
          `Mapper: could not write to destPath "${rule.destPath}" ` +
            `— ${(err as Error).message}. Field skipped.`,
        );
      }
    }
...
/** Singleton instance — import this in FanOutService instead of `new Mapper()` */
