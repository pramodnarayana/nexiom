...
describe('Mapper — simple field copy (no expression)', () => {
...
describe('Mapper — JSONata expressions', () => {
...
describe('Mapper — StitchConfig behavioral flags', () => {
...
// ─── N-to-1 Composite Mapping (Hub and Spoke) ────────────────────────────────

describe('Mapper — N-to-1 Composite Mapping (Hub to Quickbooks Vendor)', () => {
  it('maps multiple Hub canonical objects (TMS_CARRIER, TMS_TP) into a single Quickbooks Vendor', async () => {
    const compositeJson = {
      TMS_CARRIER: {
        name: 'Acme Transport',
        remit_to_address: {
          line1: '123 Carrier St',
          city: 'Atlanta',
          state: 'GA',
          postalCode: '30303',
          country: 'US'
    ...
    const rules: MappingRule[] = [
      // Name mappings
      { srcPath: 'TMS_CARRIER.name', destPath: 'DisplayName' },
      { srcPath: 'TMS_CARRIER.name', destPath: 'CompanyName' },
      // N-to-1: Using secondary object (TP) for GivenName
      { srcPath: 'TMS_TP.mc_number', destPath: 'GivenName' },
      // Address mappings
      { srcPath: 'TMS_CARRIER.remit_to_address.line1', destPath: 'BillAddr.Line1' },
      { srcPath: 'TMS_CARRIER.remit_to_address.city', destPath: 'BillAddr.City' },
      { srcPath: 'TMS_CARRIER.remit_to_address.state', destPath: 'BillAddr.CountrySubDivisionCode' },
      { srcPath: 'TMS_CARRIER.remit_to_address.postalCode', destPath: 'BillAddr.PostalCode' },
      { srcPath: 'TMS_CARRIER.remit_to_address.country', destPath: 'BillAddr.Country' },
      // Other standard fields
      { srcPath: 'TMS_CARRIER.phone', destPath: 'PrimaryPhone.FreeFormNumber' },
      { srcPath: 'TMS_CARRIER.email', destPath: 'PrimaryEmailAddr' }
    ];
    ...
    expect(payload['DisplayName']).toBe('Acme Transport');
    expect(payload['CompanyName']).toBe('Acme Transport');
    expect(payload['GivenName']).toBe('MC-123456'); // From TP
    
    const billAddr = payload['BillAddr'] as Record<string, unknown>;
    expect(billAddr['Line1']).toBe('123 Carrier St');
    expect(billAddr['City']).toBe('Atlanta');
    expect(billAddr['CountrySubDivisionCode']).toBe('GA');
    
    expect((payload['PrimaryPhone'] as Record<string, unknown>)['FreeFormNumber']).toBe('555-1234');
    expect(payload['PrimaryEmailAddr']).toBe('dispatch@acme.com');
  });
});

// ─── LRU Cache Bounding ───────────────────────────────────────────────────

describe('Mapper — LRU Cache Bounding', () => {
  it('normalizes invalid parameters (NaN, negative, fractional) to safe values', () => {
    expect((new Mapper(NaN) as any).maxCacheSize).toBe(1);
    expect((new Mapper(-5) as any).maxCacheSize).toBe(1);
    expect((new Mapper(0) as any).maxCacheSize).toBe(1);
    expect((new Mapper(0.5) as any).maxCacheSize).toBe(1); // Fractional < 1 becomes 1
    // Number higher than MAX becomes MAX
    expect((new Mapper(999999) as any).maxCacheSize).toBe(1000); 
  });

  it('evicts the oldest expression when the cache fills up', async () => {
    const smallEngine = new Mapper(2);
