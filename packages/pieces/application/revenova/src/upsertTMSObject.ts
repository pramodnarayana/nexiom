...
const metadataDictionary: Record<string, { type: string; fields: Record<string, string> }> = {
    'sf_Account': {
        type: 'TMS_CARRIER',
        fields: {
            'displayName': 'name_',
...
export const upsertTMSObject: NormalizerFn = (replica) => {
    ...
    return {
        canonicalType: meta.type as import('@nexiom/piece-framework').CanonicalType,
        sourceId: String(replica.data.Id || replica.data.id || 'unknown'),
