...
export function buildTenantSchema(schemaName: string) {
     ...
     * Engine (L6) after every successful vendor API call.
     *
     * Scoped to the source connection's workspace schema so GEM data is
     * physically co-located with the L2 replica it was derived from.
     * No cross-database FK references — stitchId / sourceAppId / destAppId
     * reference global tables logically but are enforced at the application
     * layer (not via DB constraint) to support multi-database deployments.
     */
    const globalEntityMap = schema.table('global_entity_map', {
        id: uuid('id').defaultRandom().primaryKey(),

        // Route that produced this mapping (references global integration_stitch)
        stitchId: uuid('stitch_id').notNull(),

        // ── Source side ─────────────────────────────────────────────────────
        sourceAppName: varchar('source_app_name', { length: 100 }).notNull(),
        sourceAppId: uuid('source_app_id').notNull(),
        ...
        destEntityId: varchar('dest_entity_id', { length: 255 }).notNull(),
        destRefLayer: varchar('dest_ref_layer', { length: 10 }).notNull(),
